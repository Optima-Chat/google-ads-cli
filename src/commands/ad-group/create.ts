...
export const createCommand = new Command('create')
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const result = await client.createAdGroup(options.customerId, {
        name: options.name,
        ...
        if (result && result.length > 0) {
          ...
          if (adGroupResult && adGroupResult.ad_group) {
            ...
            if (match) {
              ...
              console.log(chalk.cyan(`   google-ads keyword add -c ${options.customerId} --ad-group-id ${match[1]} --keywords "关键词1,关键词2"`));
            }
          }
        }
      }
